...
const CircuitTemplateCard = ({
  ...
  // Use a default placeholder if circuitImage is not provided
  const imageUrl = circuitImage || "/circuit-placeholder.png";
    ...
    <div>
      <div className="bg-neutral-800 rounded-xl overflow-hidden border border-neutral-700 shadow-lg hover:shadow-[#6E56CF]/10 transition-all duration-300 hover:-translate-y-1 group animate__animated animate__fadeInUp">
        <div className="relative">
          <div className="h-48 w-full bg-neutral-700 overflow-hidden">
            <img
              src={imageUrl}
              alt={`${title} circuit diagram`}
              className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                target.onerror = null;
                target.src = "/circuit-placeholder.png"; // Fallback image if the main one fails
              }}
            />
          </div>
          ...
          <div className="flex justify-between items-center">
            <div className="flex space-x-1">
              {components.map((comp: string) => (
                <span
                  key={comp}
                  className="px-2 py-1 bg-neutral-700 rounded text-xs text-neutral-400"
                >
                  {comp}
                </span>
              ))}
            </div>
            <a href={url} className="text-[#6E56CF] hover:text-white transition-colors">
              <svg
